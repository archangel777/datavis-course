function Network() {
  ...
  function showDetails(d, i) {
    ...
    content += '<p class="main"><span>' + d.artist + '</span></p>';
    tooltip.showTooltip(content, d3.event);

    return d3.select(this).style("fill", "grey").style("stroke", "black");
  }

  // Mouseout function
  function hideDetails(d, i) {
    tooltip.hideTooltip();
    return d3.select(this).style("fill", "black").style("stroke", "grey");
  }
  ...
  network = function(selection, data) {
    ...
    force.on("tick", forceTick).charge(-200).linkDistance(20);
