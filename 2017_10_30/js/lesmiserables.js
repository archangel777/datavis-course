function Network() {
  ...
  function setupData(data) {
    ...
    countExtent = d3.extent(data.nodes, d => d.neigh);
  ...
  function showDetails(d, i) {
    ...
    content += '<p class="main"><span>' + d.neigh + '</span></p>';
    tooltip.showTooltip(content, d3.event);

    return d3.select(this).style("fill", "#be29ec").style("stroke", "#660066");
  }

  // Mouseout function
  function hideDetails(d, i) {
    tooltip.hideTooltip();
    return d3.select(this).style("fill", "#660066").style("stroke", "#660066");
  }
  ...
  function updateNodes() {
    ...
    node.enter()
      .append("circle").attr("class", "node").attr("cx", d => d.x)
      .attr("cy", d => d.y)
      .attr("r", d => d.radius)
  ...
  network = function(selection, data) {
    ...
    force.on("tick", forceTick).charge(-300).linkDistance(50);
